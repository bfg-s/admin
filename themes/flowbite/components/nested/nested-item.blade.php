<ol class="dd-list">
    @foreach(($childModels ?? $models->whereNull($parent_field)) as $item)
        <li class="dd-item dd3-item" data-id="{{ $item->id }}">
            ...
            @if ($maxDepth > 1)
                @php $list = $models->where($parent_field, $item->id); @endphp
                @if($list->count())
                    @include(admin_template('components.nested.nested-item'), ['childModels' => $list])
                @endif
            @endif
        </li>
    @endforeach
